...
static const uint32_t N_SUMS = N_COUNTERS / SORT_BLK_SIZ;
...
void zero_counters()
{
    ...
    for (uint32_t i = 0; i < N_SUMS/4; i++)
        ((uint4*)counters)[threadIdx.x + i*SORT_BLK_SIZ] = uint4{0, 0, 0, 0};
#else
    #pragma unroll
    for (uint32_t i = 0; i < N_SUMS; i++)
        counters[threadIdx.x + i*SORT_BLK_SIZ] = 0;
#endif
    __syncthreads();
}
                  ...
                  uint32_t lshift, uint32_t rshift, uint32_t mask)
{
    ...
    for (uint32_t i = threadIdx.x; i < len; i += SORT_BLK_SIZ) {
                       ...
                       uint32_t histogram[])
{
    ...
    #pragma unroll 1
    for (uint32_t i = threadIdx.x; i < 1<<bits; i += SORT_BLK_SIZ)
    ...
    carry_sum = sum_up(carry_sum, SORT_BLK_SIZ/WARP_SZ);
...
void count_digits(const uint2 src[], uint32_t len, uint32_t mask)
{
    ...
    for (size_t i = threadIdx.x; i < len; i += SORT_BLK_SIZ)
              ...
              uint32_t histogram[], uint32_t wbits, uint32_t lsbits)
{
    ...
    if (wbits > DIGIT_BITS || (lg_gridDim && wbits > lg_gridDim+1)) {
        ...
        #pragma unroll 1
        for (uint32_t i = threadIdx.x; i < (uint32_t)len; i += SORT_BLK_SIZ) {

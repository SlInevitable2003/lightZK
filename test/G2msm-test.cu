...
class MSMTest {
    ...
    vector<libff::G2<ppT>> points;

    libff::G2<ppT> cpu_result, gpu_result;
public:
    MSMTest(size_t n_, bool from_binary = false) : n(n_) {
        ...
        if (from_binary) {
            ...
            std::ifstream in("G2msm_test_data.bin");
            ...
            in.read(reinterpret_cast<char*>(points.data()), n * sizeof(libff::G2<ppT>));
            ...
            for (size_t i = 0; i < n; i++) {
                ...
                points[i] = libff::G2<ppT>::random_element();
        ...
        if (from_binary) {
            ...
            std::ifstream in("G2msm_test_result.bin");
            if (!in) throw std::runtime_error("Failed to open input file.");
            in.read(reinterpret_cast<char*>(&cpu_result), sizeof(libff::G2<ppT>));
            ...
            vector<libff::G2<ppT>> results(num_threads, libff::G2<ppT>::zero());
            for (size_t i = 0; i < num_threads; i++) {
                size_t start = i * batch_size;
                size_t end = min(start + batch_size, n);
                if (start >= end) continue;
                results[i] = libff::multi_exp<libff::G2<ppT>, libff::Fr<ppT>, libff::multi_exp_method::multi_exp_method_BDLO12>(
                    points.cbegin() + start,
                    points.cbegin() + end,
            ...
            ofstream out("G2msm_test_result.bin");
            if (!out) throw runtime_error("Failed to open output file.");
            out.write(reinterpret_cast<char*>(&cpu_result), sizeof(libff::G2<ppT>));
            out.close();
...
void cuda_msm_setup(const vector<libff::Fr<ppT>> &scalars, const vector<libff::G2<ppT>> &points, MSMGPULayout &gpu_layout)
{
...
void cuda_msm_compute(MSMGPULayout &gpu_layout, libff::G2<ppT> &result)
{
